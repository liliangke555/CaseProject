//
//  CollectionViewLeftOrRightAlignedLayout.m
//  UsedCar
//
...
@interface CollectionViewLeftOrRightAlignedLayout ()
...
@end
@implementation CollectionViewLeftOrRightAlignedLayout
...
- (void)prepareLayout {
    ...
    self.viewWidth = CGRectGetWidth(self.collectionView.frame)  - self.itemInset.left - self.itemInset.right;
...
- (void)makeHeaderAttributesWithIndexPath:(NSIndexPath *)indexPath lastItemAttributes:(UICollectionViewLayoutAttributes *)attributes {
    ...
    if ( [self.delegate respondsToSelector:@selector(collectionViewDynamicHeaderSizeWithIndexPath:)] ) {
        ...
        headerWidth = CK_WIDTH - self.headerInset.left - self.headerInset.right;
...
- (void)makeFooterAttributesWithLastItemAttributes:(UICollectionViewLayoutAttributes *)attributes {
    ...
    if ( [self.delegate respondsToSelector:@selector(collectionViewDynamicFooterSizeWithIndexPath:)] ) {
        ...
        footerWidth = CK_WIDTH - self.footerInset.left - self.footerInset.right;
...
- (CGFloat)countItemSizeWidthWithIndexPath:(NSIndexPath *)indexPath{
    NSString *content = [self.delegate collectionViewItemSizeWithIndexPath:indexPath];
    CGSize size =   [content boundingRectWithSize:CGSizeMake(MAXFLOAT, self.itemHeight) options:NSStringDrawingUsesLineFragmentOrigin|NSStringDrawingUsesFontLeading attributes:@{NSFontAttributeName:self.labelFont} context: nil].size;
     
    return size.width+self.stringSpacing*2 + 5;
}
 
- (void)setContentHeight:(CGFloat)contentHeight {
    _contentHeight = contentHeight;
    if ([self.delegate respondsToSelector:@selector(backContentHeight:)]) {
        [self.delegate backContentHeight:contentHeight];
    }
}
#pragma mark - lazy
// header 的布局属性数组
