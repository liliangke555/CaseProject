                            ...
                            handler:(MMPopupInputHandler)inputHandler
{
    ...
    return [self initWithTitle:title image:nil detail:detail items:items inputPlaceholder:inputPlaceholder inputHandler:inputHandler];
}
...
- (instancetype) initWithTitle:(NSString*)title
                        detail:(NSString*)detail
                         items:(NSArray*)items
{
    return [self initWithTitle:title image:nil detail:detail items:items inputPlaceholder:nil inputHandler:nil];
}

- (instancetype)initWithTitle:(NSString *)title
                        image:(MMPopupSetImageViewHandler)imageHandler
                       detail:(NSString *)detail
                        items:(NSArray *)items
{
    return [self initWithTitle:title image:imageHandler detail:detail items:items inputPlaceholder:nil inputHandler:nil];
}
...
//                make.height.mas_equalTo(168);
        ...
        if ( self.inputHandler )
        {
            ...
            self.inputView.backgroundColor = config.inputBackgroundColor;
            self.inputView.layer.cornerRadius = config.cornerRadius;
...
//            self.inputView.layer.borderColor = config.splitColor.CGColor;
        ...
        line.backgroundColor = MMHexColor(0xDDDDDDFF);
        ...
        [self.buttonView setBackgroundColor:MMHexColor(0xDDDDDDFF)];
        
        BOOL titleTooLong = NO;
        for (MMPopupItem *item in items) {
        ...
        for ( NSInteger i = 0 ; i < items.count; ++i )
        {
            ...
            [btn mas_makeConstraints:^(MASConstraintMaker *make) {
                ...
                if ( items.count <= 2 && !titleTooLong)
                {
                    ...
                    {
                        make.left.equalTo(lastButton.mas_right).mas_offset(0.5f);
                        make.width.equalTo(firstButton);
                    }
                }
                else
                {
                    make.left.equalTo(self.buttonView).offset(16);
                    make.right.equalTo(self.buttonView).offset(-16);
                    make.height.mas_equalTo(config.buttonHeight);
                    ...
                    {
                        make.top.equalTo(lastButton.mas_bottom).offset(8);
                        make.width.equalTo(firstButton);
                    }
                }
            ...
            [btn setBackgroundImage:[UIImage mm_imageWithColor:config.itemPressedColor]
                           forState:UIControlStateHighlighted];
            [btn setBackgroundColor:MMHexColor(0xFFFFFFFF)];
            [btn setTitle:item.title forState:UIControlStateNormal];
            btn.clipsToBounds = YES;
            btn.titleLabel.font = item.highlight?KMediumFont(16):KSystemFont(16);
        }
...
- (void)actionButton:(UIButton*)btn
{
    ...
    if ( self.withKeyboard && (btn.tag==0) )
    {
...
- (instancetype)init
{
    ...
    if ( self )
    {
        ...
        self.inputBackgroundColor = MMHexColor(0xEDEDEDFF);
        self.fristItemBackgroundColor = MMHexColor(0x5064DFFF);

        self.fristItemNormalColor = MMHexColor(0xFFFFFFFF);
        self.itemNormalColor    = MMHexColor(0x999999FF);
        self.itemHighlightColor = MMHexColor(0x7FD7F2FF);
