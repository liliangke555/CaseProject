...
@interface MDYCurriculumController ()<UITableViewDelegate,UITableViewDataSource,DZNEmptyDataSetDelegate,DZNEmptyDataSetSource>
...
@implementation MDYCurriculumController
...
- (void)viewDidLoad {
    ...
    self.contentTableView.mj_header = [MDYRefreshHeader headerWithRefreshingBlock:^{
        ...
        if (weakSelf.listData.count > 0) {
            ...
            [weakSelf getFreeCourseWithId:model];
        } else {
...
- (void)getTypeData {
    MDYFreeCurriculumTypeRequest *request = [MDYFreeCurriculumTypeRequest new];
    CKWeakify(self);
...
- (void)getFreeCourseWithId:(MDYFreeCurriculumTypeModel *)typeModel {
    MDYFreeCourseRequest *request = [MDYFreeCourseRequest new];
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
    if (tableView.tag == 100) {
        ...
        MDYFreeCourseModel *model = self.dataSource[indexPath.row];
