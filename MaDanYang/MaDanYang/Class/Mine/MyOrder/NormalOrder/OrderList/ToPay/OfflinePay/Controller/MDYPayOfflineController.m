...
@interface MDYPayOfflineController ()
@property (nonatomic, strong) UIImageView *photoImageView;
@end

@implementation MDYPayOfflineController
...
- (void)createView {
    ...
    [scrollView setContentSize:CGSizeMake(0, 485 + self.orderModel.goods.count * 156)];
    
    UIView *contentView = [[UIView alloc] init];
    [scrollView addSubview:contentView];
    contentView.frame  = CGRectMake(0, 0, CK_WIDTH, 485 + self.orderModel.goods.count * 156);
    
    MASViewAttribute *lastAttribute = contentView.mas_top;
    
    for (int i = 0; i < self.orderModel.goods.count; i ++) {
        MDYPlaceOrderGoodsModel *model = self.orderModel.goods[i];
