...
@interface MDYAllGoodsController ()<UITableViewDelegate,UITableViewDataSource>
...
@implementation MDYAllGoodsController
...
- (void)viewDidLoad {
    ...
    self.contentTableView.mj_header = [MDYRefreshHeader headerWithRefreshingBlock:^{
        weakSelf.pageNum = 1;
        MDYAllGoodsTypeModel *model = weakSelf.listData[weakSelf.selectedIndex];
...
- (void)getTypeData {
    MDYAllGoodsTypeRequst *request = [MDYAllGoodsTypeRequst new];
    CKWeakify(self);
...
- (void)getAllCourseWithId:(MDYAllGoodsTypeModel *)typeModel {
    MDYAllGoodsRequest *request = [MDYAllGoodsRequest new];
    request.type_id = [typeModel.type_id integerValue];
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
    if (tableView.tag == 100) {
        ...
        MDYAllGoodsTypeModel *model = self.listData[indexPath.row];
